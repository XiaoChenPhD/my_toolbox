function y_ICC_MSb_Image_CX(Rate1Dir,Rate2Dir, Rate3Dir, OutputName,MaskFile)
...
%   Output:
%     OutputName - image with between-subject mean square (MSb, a component of Intraclass correlation coefficient)
...
% modified by ChenXiao, to add a rater and calculate the ICC among 3
% scanners
...
Rate3Series=0;
for i=1:length(Rate2Dir)
    [Temp,VoxelSize,theImgFileList, Header] =y_ReadAll(Rate3Dir{i});
    Rate3Series = Rate3Series + Temp;
    fprintf('\n\tImage Files in Rate %d Directory %d:\n',2, i);
    for itheImgFileList=1:length(theImgFileList)
        fprintf('\t%s%s\n',theImgFileList{itheImgFileList},'.img');
    end
end
Rate3Series = Rate3Series ./ (length(Rate2Dir));
